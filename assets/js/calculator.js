...
(function () {
	...
	function initEventListeners() {
		...
		if (nextStep1) {
			nextStep1.addEventListener('click', async () => await handleStep1Next());
		}
		...
		if (postcodeInput) {
			postcodeInput.addEventListener('blur', async () => await validatePostcode());
	...
	/**
	 * Handle Step 1 Next button WORKING
	 * This method validates the postcode and property value inputs.
	 * It uses the validatePostcode function to check the postcode against the server, 
	 * 	and ensures the property value is a valid number within configured limits. 
	 * If validation passes, it saves the data to state and navigates to step 2.
	 */
	async function handleStep1Next() {
		...
		if (!numericValue || isNaN(numericValue)) {
			...
			hideError('property-value-error');
		}
	...
	function handleCalculate() {
		...
		if (agePrimary < minAge) {
			showError('age-primary-error', 'Age must be at least ' + minAge);
			return;
		}
		...
		if (calculatorState.data.hasPartner && agePartner) {
			...
			if (
				isNaN(agePartner) || agePartner < minAge
				//	|| agePartner > maxAge
			) {
				showError(
					'age-partner-error',
					//'Partner age must be between ' + minAge + ' and ' + maxAge
					'Partner age must be at least ' + minAge
				);
				return;
			}
		} else {
	...
	/**
	 * Validate postcode via AJAX WORKING
	 * This method validates against postcodes.php via AJAX.
	 * It handles the response to determine if the postcode is valid, ineligible, or requires referral, and updates the UI accordingly.
	 */
	async function validatePostcode() {
		...
		try {
			const response = await fetch(m60CalcConfig.ajaxUrl, {
				method: 'POST',
				body: data
			});
			...
			if (!result.success) {
				showError('postcode-error', result.data.message || 'Error validating postcode');
			...
			if (result.data.valid) {
				...
				if (result.data.message) {
					showError('postcode-error', result.data.message);
				}
				return true;
			}
			...
			showError('postcode-error', error.message || 'Error validating postcode');
