...
class Money_At_60_Calculator
{
	...
	public function enqueue_frontend_assets()
	{
		...
		wp_enqueue_style(
			'm60-calculator-css',
			M60_CALC_PLUGIN_URL . 'assets/css/calculator.min.css',
			array('bootstrap'),
			M60_CALC_VERSION
		);
		...
		wp_enqueue_script(
			'm60-calculator-js',
			M60_CALC_PLUGIN_URL . 'assets/js/calculator.min.js',
			array(),
			M60_CALC_VERSION,
			true
		);
	...
	public function enqueue_admin_assets($hook)
	{
		...
		wp_enqueue_style(
			...
			M60_CALC_PLUGIN_URL . 'assets/css/admin.min.css',
			array(),
			M60_CALC_VERSION
		);
	}
	...
	public function ajax_validate_postcode()
	{
		...
		$postcode = sanitize_text_field($_POST['postcode']);
		$calculator = new M60_Calculator_Engine();
		$postcode_label = $calculator->get_postcode_status($postcode);
		$is_valid = $postcode_label !== false;
		// default validation message
		$postcode_message = '';

		if (!$postcode_label) {
			$postcode_message = 'Sorry, that postcode is not valid or we do not currently service that area.'; // Override
			$is_valid = false; // Mark as invalid
		} elseif ($postcode_label === 'Ineligible') {
			$postcode_message = 'Unfortunately this postcode is not in an area currently serviced by our lenders.'; // Override
			$is_valid = false; // Mark as invalid
		} elseif ($postcode_label === 'Refer') {
			$postcode_message = 'This postcode requires lender validation. Approval, if granted, may be subject to a lower maximum LVR.';  // Override
			$is_valid = true; // Mark as invalid
		} elseif ($postcode_label === 'SomeOtherStatus') {
			$postcode_message = 'Some other message'; // Example of handling another status
			$is_valid = false; // Mark as invalid
		};

		wp_send_json_success(array(
			'valid' => $is_valid,
			'message' => $postcode_message
		));
	}
